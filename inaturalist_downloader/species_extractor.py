"""Extract fish species lists from iNaturalist species-count queries.

This module powers the `inat-extract-species` CLI and the backward-compatible
`extract_species_from_place.py` wrapper. It resolves a target place, resolves
fish family names, queries iNaturalist for species counts, and writes both a
plain species list and a TSV count report.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        Parsed CLI options. The namespace is also passed into helper functions
        that need filtering settings such as quality grade, pagination, and
...
def http_get_json(path: str, params: Optional[dict] = None, retries: int = 5) -> dict:
    ...
    Args:
        path: API path relative to `API_BASE`, for example
            `/observations/species_counts`.
        params: Optional query string parameters. List-like values are encoded
            with `doseq=True`.
...
def choose_best_result(results: list[dict], query: str, name_keys: list[str]) -> dict:
    ...
    Matching preference is exact case-insensitive match, then partial
    case-insensitive match, then the first API result.

    Args:
        results: Autocomplete result objects returned by iNaturalist.
        query: Original user query, such as a place, family, or taxon name.
        name_keys: Result fields to compare against the query.
...
def resolve_place(place_query: str) -> tuple[int, str]:
    ...
    Args:
        place_query: Place name to search through `/places/autocomplete`.

    Returns:
        Tuple of `(place_id, display_name)`.

    Raises:
        ValueError: If no matching place is returned.
    """
...
def resolve_taxon(taxon_query: str) -> tuple[int, str]:
    ...
    Args:
        taxon_query: Scientific/common taxon query, usually a fish family name.

    Returns:
        Tuple of `(taxon_id, resolved_scientific_name)`.

    Raises:
        ValueError: If no matching taxon is returned.
    """
...
def fetch_species_counts(args: argparse.Namespace, place_id: Optional[int]) -> list[dict]:
    ...
    The current taxon is read from `args.taxon_id`. In the family workflow this
    value is temporarily set to each resolved family ID before calling this
    function.

    Args:
        args: Parsed CLI options containing pagination and filtering settings.
        place_id: iNaturalist place ID. `None` means global search.

    Returns:
        Species rows sorted by descending observation count, then taxon name.
        Each row contains `taxon_id`, `name`, `rank`, `count`, and
...
def get_species_for_place(
    ...
    Each family name is resolved to a taxon ID, then `fetch_species_counts`
    retrieves matching species under that family and location. Families that
    cannot be resolved are skipped with a warning.

    Args:
        args: Parsed CLI options.
        place_id: iNaturalist place ID. `None` means global search.
        place_name: Human-readable label used in logs.
        families: Family names loaded from `family.txt` or `--families-file`.

    Returns:
        Combined species rows from all successfully resolved families.
    """
...
def write_species_list(path: Path, species_rows: list[dict]) -> None:
    ...
    Args:
        path: Destination path, usually `species.txt`.
        species_rows: Filtered species records returned by the extraction flow.
    """
...
def write_counts_tsv(path: Path, species_rows: list[dict]) -> None:
    ...
    Args:
        path: Destination path, usually `<species file stem>_counts.tsv`.
        species_rows: Filtered species records returned by the extraction flow.
    """
...
def main() -> None:
    ...
    The workflow validates arguments, loads family names, builds a fallback
    chain of locations, queries each family in each location until enough
    species are found, then writes `species.txt` and a count TSV.
    """
    args = parse_args()
